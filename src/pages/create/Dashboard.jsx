...
import { Plus, ChevronRight, Users, Calendar, BookOpen, Search, Edit, Trash2, GraduationCap, Download, FileUp, PlusCircle, TicketCheckIcon, TicketXIcon, ReceiptTextIcon } from 'lucide-react';
...
//   const [newYear, setNewYear] = useState({ year: '', status: 'Active' });
  ...
  const sessionalResult = async (sessionId, departmentId)=>{
    const results =  await window.api.result.handleExport(sessionId, departmentId, `${selectedYear.start_year}/${selectedYear.end_year}`)
  ...
  const renderBreadcrumb = () => (
    <div className='w-full flex items-center justify-between'>
        <div className="flex items-center gap-2 text-sm text-gray-600 mb-6">
            <button onClick={() => setCurrentStep('main')} className="hover:text-blue-600">Dashboard</button>
            {selectedFaculty && (
                <>
                <ChevronRight size={16} />
                <button onClick={() => setCurrentStep('year')} className="hover:text-blue-600">{selectedFaculty.name}</button>
                </>
            )}
            {selectedDepartment && (
                <>
                <ChevronRight size={16} />
                <button onClick={() => setCurrentStep('main')} className="hover:text-blue-600 w-32 overflow-clip text-nowrap">{selectedDepartment.name}</button>
                </>
            )}
            {selectedYear && (
                <>
                <ChevronRight size={16} />
                <span onClick={()=> setCurrentStep('year')} className="text-gray-900 hover:underline cursor-pointer">{selectedYear.start_year} / {selectedYear.end_year}</span>
                <ChevronRight size={16} />
  ...
  const renderMainDashboard = () => (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {
            selectedFaculty && (
                <div className='w-full shadow my-4'>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {academicYears?.length > 0 && academicYears?.map((year) => (
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h3 className="font-semibold text-gray-900 mb-2">{year.start_year} / {year.end_year}</h3>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        ...
                        <div key={sem.id}>
                            ...
                            <div className='space-y-2'>
                                {
                                    key === 0 ? (
                                        firstSmesterCourses && firstSmesterCourses.map((item)=>{
                                            return(
                                                <div key={item.id} className='space-y-2 flex items-center justify-between border shadow-md py-2 px-6 rounded-md'>
                                                    <div className='font-semibold flex flex-col gap-2 h-full items-start justify-center'>
                                                        <div className='h-full'>{item.name} {item.code} <span className='font-normal'>(unit - {item.unit})</span></div>
                                                        {/* <div> unit - {item.unit} </div> */}
                                                    </div>
                                                    <div>
                                                      <Trash2 className='text-red-400 cursor-pointer' onClick={()=> removeCourseFromSemester(item.semester_id, item.course_id)}/>
                                                    </div>
                                                </div>
                                            )
                                        })
                                    ) : (
                                        secondSmesterCourses && secondSmesterCourses.map((item)=>{
                                            return(
                                                <div key={item.id} className='space-y-2 flex items-center justify-between border shadow-md py-2 px-6 rounded-md'>
                                                    <div className='font-semibold flex flex-col gap-2 h-full items-start justify-center'>
                                                        <div className='h-full'>{item.name} {item.code} <span className='font-normal'>(unit - {item.unit})</span></div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
          <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg px-4 py-2 text-white">
            <div className="flex items-center justify-between">
              <div>
          ...
          <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-lg px-4 py-2 text-white">
            <div className="flex items-center justify-between">
              <div>
          ...
          <div className="bg-gradient-to-br from-amber-500 to-amber-600 rounded-lg px-4 py-2 text-white">
            <div className="flex items-center justify-between">
              <div>
        ...
        <div className="grid grid-cols-12 mb-6 flex-wrap">
          ...
          <div className='col-span-10 flex items-center flex-wrap gap-4'>
            <div className='w-44'>
              <Select value={studentFilter} onValueChange={(val) => setStudetFilter(val)}>
                  <SelectTrigger><SelectValue className='w-32' placeholder="Select Course" /></SelectTrigger>
                <SelectContent>
            ...
            <div onClick={()=> sessionalResult(selectedYear.id, selectedDepartment.id)} className="flex cursor-pointer items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition">
              <Download size={18} />
              Download Result
            </div>
            <div onClick={()=> setShowMarksheet(true)} className="flex cursor-pointer items-center gap-2 bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700 transition">
              <Download size={18} />
              Download Mark Sheet
            </div>
            <div onClick={()=> setShowUploadModal(true)} className="flex cursor-pointer items-center gap-2 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition">
              <FileUp size={18} />
              Upload students
            </div>
            <div onClick={()=> setShowStudentModal(true)} className="flex cursor-pointer items-center gap-2 bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 transition">
              <Plus size={18} />
              Add Student
            </div>
          </div>
        ...
        {/* search */}
        <StudentSearch departmentId={selectedDepartment.id} onResults={setStudents} fetchAll={fetchStudents}/>

        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Matric No.</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {students.map((student) => (
                <tr key={student.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-sm text-gray-900">{selectedFaculty?.code}{student.matric_no}</td>
                  <td className="px-4 py-3 text-sm text-gray-900 font-medium">{student.first_name} {student.last_name}</td>
                  ...
                  <td className="px-4 py-3 text-sm">
                    <div className="flex gap-2 items-center">
                      ...
                      <div className='w-32'>
                        <Select value={student.status} onValueChange={(val) => changeStatus(student.id, val)}>
                            <SelectTrigger><SelectValue className='w-32' placeholder="Select Course" /></SelectTrigger>
                          <SelectContent>
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      {showFacultyModal && (
        <div className="absolute inset-0 bg-gray-200 bg-opacity-50 flex items-center justify-center z-50">
            <div className='min-w-md max-w-md'>
                ...
                className="w-full text-center cursor-pointer flex-1 px-4 py-2 border-2 border-gray-300 rounded-lg hover:bg-gray-50 transition"
              >
                Close
              </div>
            </div>
      ...
      {showStudentModal && (
        <div className="absolute inset-0 bg-white overflow-y-scroll bg-opacity-50 flex items-center justify-center z-50">
            <div className='min-w-md max-w-md'>
                <StudentPage facultyId={selectedFaculty && selectedFaculty.id} department={selectedDepartment} level={level} onComplete={fetchStudents}/>
                <div
      ...
      {showUploadModal && (
        <div className="absolute inset-0 bg-white overflow-y-scroll bg-opacity-50 flex items-center justify-center z-50">
            <div className='min-w-md max-w-md'>
                <ExcelUploader departmentId={selectedDepartment && selectedDepartment.id} facultyId={selectedFaculty && selectedFaculty.id} onClose={filterStudents}/>
                <div
