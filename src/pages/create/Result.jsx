...
import { 
  Save, Loader, CheckCircle, AlertCircle, BookOpen, User, Calendar, 
  ToggleLeft, ToggleRight, Clock, Ban, FileX, FileCheck 
} from 'lucide-react';
import { Select, SelectTrigger, SelectSeparator, SelectItem, SelectValue, SelectContent } from '@/components/ui/select';
import CarryoverModal from '../students/CarryOvers';
import { parseCarryoverString } from '@/utils/generateResultSheet';

const ResultUpdateComponent = ({ student, session, onResultsUpdate, firstSemesterCourses, secondSmesterCourses, courses }) => {
  const [semesters] = useState({ first: firstSemesterCourses || [], second: secondSmesterCourses || [] });
  ...
  const fetchStudentResults = async () => {
    ...
    setLoading(true);
    try {
      const data = await window.api.result.getStudentResults(student.id, session.id, activeSemester === 'first'? 1 : 2) 
      setResultValue(data || [])
    } catch (error) {
...
//   };
  ...
  const calculateGrade = (score) => {
    if (score >= 70) return { grade: 'A', points: 5.0, color: 'text-green-600', bgColor: 'bg-green-50' };
    if (score >= 60) return { grade: 'B', points: 4.0, color: 'text-blue-600', bgColor: 'bg-blue-50' };
    if (score >= 50) return { grade: 'C', points: 3.0, color: 'text-yellow-600', bgColor: 'bg-yellow-50' };
    if (score >= 45) return { grade: 'D', points: 2.0, color: 'text-orange-600', bgColor: 'bg-orange-50' };
  ...
  const getStatusConfig = (status) => {
    const configs = {
      ...
      NR: { label: 'Not Registered', color: 'text-orange-700', bgColor: 'bg-orange-100', icon: ToggleLeft }
    };
  ...
  const CourseCard = ({ course, semester }) => {
    ...
    const gradeInfo = resultVal?.score ? calculateGrade(resultVal?.score) : score && status === 'valid' ? calculateGrade(score) : null;
    const saveStatusVal = saveStatus[resultKey] || 'idle';

    return (
      <div className={`bg-white col-span-1 rounded-lg border p-4 transition-all ${
        isCarryover ? 'border-orange-300 bg-orange-50' : 'border-gray-200 hover:shadow-md'
      }`}>
        {/* Course Header */}
        <div className="flex items-center justify-end w-full">
          <div className="flex items-center space-x-2 ml-4">
            {getSaveStatusIcon(saveStatusVal)}
          </div>
        </div>
        ...
        <div className="flex items-center space-x-4">
          <div className="flex-1">
            <div className="flex items-center space-x-2 mb-1">
              <h3 className="font-bold text-gray-900">{course.code} - {course.name}</h3>
              {isCarryover && (
          ...
          {status === 'valid' && (
            <div className="flex-1">
              ...
              <Select value={resultVal?.score || result.score} onValueChange={(val) => handleScoreChange(course.id, Number(val), semester, resultVal || result || {})}>
                    ...
                    <SelectContent className='h-60 overflow-y-scroll'>
                        {
                            Array.from({length: 101}, (_, index)=>(
                                <SelectItem value={parseInt(index)} key={index}>{index}</SelectItem>
                            ))
                        }
                    </SelectContent>
                </Select>
          ...
          {status === 'valid' && (
            <div className="flex-1">
              ...
              <Select value={resultVal?.ca || result.ca} onValueChange={(val) => handleCAChange(course.id, Number(val), semester, resultVal || result || {})}>
                    <SelectTrigger className='col-span-3 w-32'>
          ...
          {status === 'valid' && gradeInfo && (
            <>
              <div className="text-center">
                ...
                <div className={`px-3 py-2 font-bold rounded-md ${gradeInfo.color} ${gradeInfo.bgColor}`}>
                  {gradeInfo.grade}
                </div>
              </div>
  ...
  return (
    <div className="px-6 pt-12 relative">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="p-6">
          <SemesterTab 
            ...
            courses={semesters[activeSemester]} 
          />
        </div>
      </div>
      ...
      <CarryoverModal studentId={student.id} onClose={()=> { setShowCarryovers(false); getCarryovers()}} isOpen={showCarryovers} courses={courses} carryovers={carryovers}/>
    </div>
  );
};
