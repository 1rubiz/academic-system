...
import { getStudentsFiltered, filterStudents } from "../studentUtils.js";
...
export function registerStudentHandlers() {
  ...
  ipcMain.handle("add-student", async (_, student) => {
    try {
      const { matric_no, first_name, last_name, department_id, faculty_id, level = 100, status = "active" } = student;
  ...
  ipcMain.handle("bulk-upload-students", (_, students, faculty_id, department_id, activeLevel = 100) => {
      try {
        const insertStmt = db.prepare(`
          INSERT INTO students 
          (
        ...
        db.transaction(() => {
          students.forEach((s) => {
            ...
            // activeLevel logic
            const activeFlags = [100, 200, 300, 400, 500, 600].map(
              (lvl) => lvl === activeLevel ? 1 : 0
            );
            ...
            if (existing) {
              const sameName =
                ...
                (existing.middle_name || "").toLowerCase() ===
  ...
  ipcMain.handle("set-student-level-active", async (_, id, level) => {
    ...
    const current = db.prepare(
      `SELECT ${field} FROM students WHERE id = ?`
    ).get(id);
