...
import { createSession,
  // getSessions,
  computeGPA, addSessionResult } from "../sessionsUtil.js";
...
export function registerSessionHandlers() {
  ...
  ipcMain.handle("get-session", async (_, id) => {
    ...
    return stmt.all(id).map((s) => ({
      ...s,
      semesters: semesters.filter((sem) => sem.session_id === s.id),
      semester_courses: checkStmt.filter((cos)=> cos.id === semesters.filter((sem) => sem.session_id === s.id)[0].id || cos.id === semesters.filter((sem) => sem.session_id === s.id)[1].id)
    }));
