...
export const addResults =  (resultData) => {
  ...
  const student = db.prepare("SELECT id, department_id FROM students WHERE matric_no = ?").get(matric_no);
...
export const getStudentResult = async ({ studentId, sessionId }) => {
  const firstSemester = db.prepare(`
    SELECT 
  ...
  const secondSemester = db.prepare(`
    SELECT 
      c.id, c.code, c.name, c.unit, 
      d.name as department_name,
      sc.id as session_course_id
