...
import { db, markSheetpath } from "../main.js";
  ...
  try {
    ...
    for (const r of results) {
      ...
      if (!students[r.matric_no]) {
        const carry = db.prepare(`SELECT courses FROM carryover WHERE student_id = ?`).get(r.student_id);
    ...
    Object.values(students).forEach((student, index) => {
      ...
      const category = getSenateCategory(parseFloat(gpa), student.carryovers, student.failed_courses_str, student.senate_status);
